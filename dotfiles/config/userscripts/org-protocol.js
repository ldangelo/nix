// ==UserScript==
// @name         org-protocol
// @namespace    https://github.com/xuchunyang/setup-org-protocol-on-mac
// @version      0.0
// @description  Use Alt + l for Org: store-link and use Alt + o for Org: capture
// @author       Chunyang Xu
// @match        *://*/*
// @grant        none
// ==/UserScript==

(function () {
  "use strict";
  document.addEventListener(
    "keydown",
    function (e) {
      console.info(e);

      // Alt + c
      if (
        e.keyCode == 67 &&
        !e.shiftKey &&
        !e.ctrlKey &&
        e.altKey &&
        e.metaKey
      ) {
        window.location =
          "org-protocol://store-link?url=" +
          encodeURIComponent(document.location.toString()) +
          "&title=" +
          encodeURIComponent(document.title.toString());
      }
      //
      // Alt + o
      if (
        e.keyCode == 79 &&
        !e.shiftKey &&
        !e.ctrlKey &&
        e.altKey &&
        e.metaKey
      ) {
        var url = document.location.toString();
        var title = document.title.toString();
        var selection = window.getSelection();
        var org_capture_url =
          "org-protocol:/capture://W/" +
          encodeURIComponent(url) +
          "/" +
          encodeURIComponent(title) +
          "/" +
          encodeURIComponent(selection);
        window.location = org_capture_url;
      }

      // Allt + r
      if (
        e.keyCode == 82 &&
        !e.shiftKey &&
        !e.ctrlKey &&
        e.altKey &&
        e.metaKey
      ) {
        var op_url = document.location.toString();
        var op_title = document.title.toString();

        var org_protocol_capture_url =
          "org-protocol:/roam-ref?template=R&ref=" +
          encodeURIComponent(op_url) +
          "&title=" +
          encodeURIComponent(op_title) +
          "&body=" +
          encodeURIComponent(window.getSelection);
        window.location = org_protocol_capture_url;
      }
    },
    false,
  );
})();
